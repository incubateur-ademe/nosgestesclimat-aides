...
import { Aide as AideDB } from "../../../generated/prisma/client";
    ...
    WithPartenaireCodes<AideDefinitionEtPartenaires>
{
  ...
  public async loadCache() {
    ...
    let liste_aides;
    if (App.isProd()) {
      liste_aides = await this.prisma.client.aide.findMany({
        where: {
          VISIBLE_PROD: true,
        },
      });
  ...
  public async findByPartenaireId(
    ...
    const result = await this.prisma.client.aide.findMany({
      where: {
  ...
  public async updateCodesFromPartenaireFor(
    ...
    await this.prisma.client.aide.update({
  ...
  async upsert(aide: AideDefinition): Promise<void> {
    ...
    await this.prisma.client.aide.upsert({
  ...
  async delete(content_id: string): Promise<void> {
    await this.prisma.client.aide.delete({
      where: { content_id: content_id },
    });
  }

  async getByContentIdFromDB(content_id: string): Promise<AideDefinition> {
    const result = await this.prisma.client.aide.findUnique({
      where: { content_id: content_id },
    });
  ...
  async listeAll(): Promise<AideDefinitionEtPartenaires[]> {
    const liste_aides = await this.prisma.client.aide.findMany();
  ...
  async countAll(): Promise<number> {
    const count = await this.prisma.client.aide.count();
    return Number(count);
  }
  ...
  async listePaginated(
    ...
    const results = await this.prisma.client.aide.findMany({
      skip: skip,
      take: take,
      orderBy: {
        content_id: "desc",
  ...
  async search(filter: AideFilter): Promise<AideDefinition[]> {
    const query = this.buildSearchQuery(filter);
    const result = await this.prisma.client.aide.findMany(query);
