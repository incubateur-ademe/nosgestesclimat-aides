...
import { AideExpirationWarning } from "../../../generated/prisma/client";
...
export class AideExpirationWarningRepository {
  ...
  public async delete(cms_id: string) {
    try {
      await this.prisma.client.aideExpirationWarning.delete({
  ...
  public async get(cms_id: string): Promise<AideWarningDefinition> {
    const result = await this.prisma.client.aideExpirationWarning.findUnique({
      where: { aide_cms_id: cms_id },
    });
    if (result) {
      return {
  ...
  public async get_all(): Promise<AideWarningDefinition[]> {
    const result = await this.prisma.client.aideExpirationWarning.findMany();
    return result.map((r) => ({
  ...
  public async upsert(warning: AideWarningDefinition) {
    ...
    await this.prisma.client.aideExpirationWarning.upsert({
      where: {
