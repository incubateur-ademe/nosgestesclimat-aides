...
import { BlockText } from "../../../generated/prisma/client";
...
export class BlockTextRepository {
  ...
  async loadCache(): Promise<void> {
    const result = await this.prisma.client.blockText.findMany();
  ...
  async upsert(block: BlockTextDefinition): Promise<void> {
    ...
    await this.prisma.client.blockText.upsert({
      where: {
        id_cms: block.cms_id,
      },
      create: {
  ...
  async delete(cms_id: string): Promise<void> {
    await this.prisma.client.blockText.delete({
      where: {
        id_cms: cms_id,
      },
    });
